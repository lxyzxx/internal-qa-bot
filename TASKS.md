...
项目已经达到“可本地演示的增强 MVP”状态：内部知识问答主链路、问题分层、混合检索、来源追踪、权限过滤、自动评估、用户反馈、运行监控、基础前端和可选 Qdrant 向量检索已经落地。

项目还没有达到“公司内部生产上线”状态：登录态、完整管理端、复杂文档解析、真实业务系统工具调用、审计和生产部署仍未完成。
...
- [x] 文档可见角色和问答权限过滤
- [x] 用户反馈接口：`POST /api/feedback`、`GET /api/feedback`
- [x] 自动评估接口：`POST /api/evaluations/run`
- [x] 运行监控接口：`GET /api/metrics`
